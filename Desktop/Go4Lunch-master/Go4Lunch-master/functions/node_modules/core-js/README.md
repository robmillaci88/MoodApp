...
- ES6 logic for `@@isConcatSpreadable` and `@@species` (in most pojoPlaces) can be polyfilled without problems, but it will cause a serious slowdown in popular cases in some engines. It will be polyfilled when it will be implemented in modern engines.
